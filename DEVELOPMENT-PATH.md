...
| R10c | Device- and precision-matched replication of the R10b profile | Launched — interpretation predeclared | The R10b stage-A gate ran on local CPU at fp32 while R10's decisive learner was one T4 at fp16, and every stage-A certificate says "the same fixed learner". This row runs the *unchanged* R10b profile on that learner: same four contract hashes, same distinct-episode accounting, same per-family seeds, same grouped objective, same 64-update / 256-presentation budget, same barrier of predeclared exact full-support fit. Only the device, its precision, and the wall-clock bounds that follow differ, and the validator admits exactly those two spellings and no third. Configuration: `configs/r10b/decomposition_gate_t4.toml`. Predeclared before launch: all four families reach exact full-support fit means the R10b CPU outcome was a device/precision artifact and the four certificates are not falsified, with each stage-A family frontier for its own basic relation only; none reaching it means the falsification stands on the matched learner; a mixture is read per family with no aggregate claim; and any incomplete, non-finite, ABI/bounds, or timeout outcome is `unscored`, an apparatus defect whose receipt is preserved rather than retried. No branch admits a composite, reopens R10, authorizes R11, or permits tuning this profile. The R10b CPU receipt is preserved either way. |
