...
| R10 | Four-trunk seed portfolio | Ready for bounded Kaggle pilots | Conditions 2, 3, 5, 6, and 7 are fixed: one mixed PyO3 boundary, audited brackets, deduplicated mixture accounting, declared comparators/sentinels, and immutable cadence/budget/stop contracts. The selected-core CPU timing gate stopped unscored: four dynamically padded updates took 8.43 s against the fixed 3 s limit. Conditions 1 and 4 await the fixed five-family one-T4 run; its exact-SHA Kaggle dispatch and reachable public `origin` are configured. |
