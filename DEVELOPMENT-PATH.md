...
| R3c | Typed embodied-world compiler spike | Experimental — isolated | A separate `codex/term-compiler-spike` worktree carries a finite typed term compiler and deterministic bounded generator. It is not wired into cards, renderers, or learner profiles. The spike is retained only if its differential/admission follow-up shows that it preserves existing semantics without imposing a card ontology. |
