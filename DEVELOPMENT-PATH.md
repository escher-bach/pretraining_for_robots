...
| R3c | Typed embodied-world compiler spike | Experimental — isolated generated-family boundary | A separate `codex/term-compiler-spike` worktree carries a finite typed term compiler, exact action/scoring/prelude/reveal semantics, and deterministic receipt-filtered embodiment families. It remains outside cards, renderers, and learner profiles. The version-2 public trace remains frozen: it emits one opaque, non-injective norm token, including the known `AfterStep(3)`/`Never` collision, so prior traces and receipts replay unchanged. A visibility-gated structured symbolic goal diagnostic is derived from the norm and checked invertible, but is not a learner carrier until a separately versioned framed event profile lowers it through the canonical boundary. Norm references are validated from the denotation itself, including private norms and `OnCellEntry` guards. Generated composite-goal admission remains deferred pending an exact denotation-level nondegeneracy witness. The Card 03 differential harness covers all 12 contracts and every 25-sequence transition/outcome table, including restoration; rendering, ambiguity/orbit parity, and any admission claim remain follow-up gates. |
...
| R13 | Dependent composition, prompting, and grounding | Evidence-gated — interface contract isolated | Cards 01/07/08 and visual or physical descendants remain gated by R11–R12 evidence. The isolated `experiments/prompted-interface` fixture now makes the minimum future-facing sequence boundary executable: public goal carriers, prompt/target phases, actor- and embodiment-scoped ports, prompt-only demonstrations, target-only action supervision, and separate private outcome denotation. Its conformance tests reject source-action leakage and identity-copy shortcuts. This is apparatus-contract evidence only; the production event ABI, training batches, and language/vision adapters do not yet implement the contract. |
