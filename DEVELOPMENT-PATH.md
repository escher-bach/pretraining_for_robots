...
| R10b | Stage-A decomposed families and their own gate | Complete — `decomposition_gate_incomplete`; every certificate's falsifier fires | The four decomposition certificates in `CARDS.md` are implemented as new audited family versions with their own contract hashes: `card04a` `06a36ac33c3f952b` (16 cases, 16 distinct episodes), `card03a` `235c0a6ad2efb10d` (12/12), `card05a` `4941b2e1e1c8c390` (12/6), and `card06a` `b6f762bb89601096` (20/14). Each passes its own semantic audit: exact enumeration, preserving and meaning-changing orbits, information orbits where the value orbit is blind, non-interference, an isolating baseline bracket, and learner round-trips. The gate ran under `configs/r10b/decomposition_gate.toml`, a newly declared AdmissionProfile that inherits the decisive R10 repair's learner core, grouped objective, batch, and 64-update / 256-presentation budget and changes only the world; its barrier is the one every certificate states for itself, predeclared exact full-support fit, not R10's heuristic `0.80` / `0.25` / `0.60` numbers, which remain binding for R10 alone. Local CPU, 469.1 s, preflight passed, exactly 256 presentations per family. Macro case-kind argmax: card04a `0.3750 -> 0.3750`, card03a `0.5000 -> 0.8333`, card05a `0.8333 -> 0.8333`, card06a `0.2500 -> 0.7500`. No family reached exact full-support fit, and in card03a and card05a both controls reached `1.0000` while the witness held at `0.5000`. All four are `support_fit_incomplete`. The classification is learner/support fit: the audits and the apparatus pass — ABI, bounds, exact accounting, finite parameters, no timeout, and falling training loss on three of the four — and the fixed profile misses its barrier. That falsifies what the four certificates predicted: the factors they removed are not what the composites' difficulty turned on. The receipt records that all four would also miss R10's barrier, so the outcome is not an artifact of which barrier was declared. One deviation is recorded rather than smoothed over: this learner is cpu/fp32 where R10's was cuda/fp16, so the pairing is not byte-identical. No composite is admitted, R10 is not reopened, and R11 stays blocked. The profile's predeclared action forbids tuning it. Compact evidence: `audit/runs/pretraining-r10b-stage-a-decomposition-gate-local/receipt.json`. |
| R10c | Device- and precision-matched replication of the R10b profile | Complete — `decomposition_gate_incomplete` on the matched learner | The R10b stage-A gate ran on local CPU at fp32 while R10's decisive learner was one T4 at fp16, and every stage-A certificate says "the same fixed learner". This row ran the *unchanged* R10b profile on that learner — same four contract hashes, distinct-episode accounting, per-family seeds, grouped objective, 64-update / 256-presentation budget, and exact-full-support-fit barrier, with only the device, its precision, and the wall-clock bounds differing, and a test asserting the two configurations differ in nothing else. Audit-verified at source `c2b60907c9ce61a5e73ba5a1c0ab9ae7ba72d488` on a real Tesla T4 with a clean tree, remote Rust and 67 Python tests passing, CUDA preflight passed (four updates `3.3810 s`, full-corpus evaluation `0.6577 s`), and exactly 256 presentations per family. Macro case-kind argmax: card04a `0.3750 -> 0.7500`, card03a `0.5000 -> 0.8333`, card05a `0.8333 -> 0.8333`, card06a `0.2500 -> 0.7500`. **No family reached exact full-support fit**, so the predeclared "none" branch applies: the falsification stands on the matched learner and what the four certificates predicted is wrong. card03a, card05a, and card06a reproduce the CPU numbers exactly at every rung; card04a is the only family the device moved, from flat at `0.3750` to `0.7500`, and it still misses the barrier — with its goal-predictable control at `1.0000`, its witness at `0.7500`, and its constant-goal control at `0.5000`, so the bracket the family was built to produce is not restored either. All four would also miss R10's barrier. No composite is admitted, R10 is not reopened, R11 stays blocked, and the profile is not tuned. Compact evidence: `audit/runs/pretraining-r10c-stage-a-c2b6090/receipt.json`; run: `https://www.kaggle.com/code/aniruddhavarma/pretraining-r10c-stage-a-c2b6090`. The gate receipt's `row` field reads `R10b` because it is emitted from the profile rather than from the chart row; this is chart row R10c executing the unchanged R10b profile. |
